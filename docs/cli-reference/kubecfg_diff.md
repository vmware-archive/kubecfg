...
Display differences between server and local configuration.

ksonnet applications are accepted, as well as normal JSON, YAML, and Jsonnet
files.

```
kubecfg diff [<env>|-f <file-or-dir>]
```

### Examples

```
  # Show diff between resources described in a local ksonnet application and
